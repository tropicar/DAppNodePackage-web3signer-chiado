...
function send_dappmanager_notification() {
  curl -X POST -G 'http://my.dappnode/notification-send' --data-urlencode 'type=danger' --data-urlencode title="$ETH2_CLIENT chiado Chain is not available" --data-urlencode 'body=Make sure you select an available client in the web3signer at packages > web3signer chiado Chain > config > chiado Chain Consensus Layer Client'
}
...
function get_web3signer_status() {
  ...
  response=$(curl -s -w "%{http_code}" -X GET -H "Content-Type: application/json" -H "Host: web3signer.web3signer-chiado.dappnode" "${WEB3SIGNER_API}/upcheck")
...
function get_web3signer_pubkeys() {
  local response content http_code
  response=$(curl -s -w "%{http_code}" -X GET -H "Content-Type: application/json" -H "Host: web3signer.web3signer-chiado.dappnode" "${WEB3SIGNER_API}/eth/v1/keystores")
  http_code=${response: -3}
  content=$(echo "${response}" | head -c-4)
  response_middleware "$http_code" "$content" "web3signer"
